...
class NetworkingClient(object):
    def __init__(self, ip, server_broadcast_port, server_play_broadcast_port, client_broadcast_port, buffer_size=1024):
        ...
        self._server_play_broadcast_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # UDP
        self._server_play_broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
        self._server_play_broadcast_socket.bind(("", server_play_broadcast_port))  # Bind to all
        self._server_play_broadcast_socket.settimeout(5.0)
    ...
    def receive_server_play_broadcast(self):
        logger.debug("waiting for server play broadcast message ...")
        ...
        try:
            ...
            logger.debug("Server play broadcast received: %s", msg)
            return msg
